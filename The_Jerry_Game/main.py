...
class Windows(tk.Frame):
    ...
    def initializer(self):  # ACA INICIALIZAMOS EL ARDUINO
        # DEBEN CAMBIAR EL PUERTO A SU PUERTO CORRESPONDIENTE
    ...
    def widget(self):
        ...
        font_color = "#FFFFFF"
        self.entry_name_label = tk.Label(
            self, text="Name:", bg=background_color, fg=font_color)
        self.entry_name_label.place(x=0, y=0)
        self.entry_name = tk.Entry(self)
        self.entry_name.place(x=50, y=3)
        self.start_button = tk.Button(self, text="Start", width=7, command=lambda: (
            self.str_name(), self.Jerry_Game()), bg=background_color, fg=font_color)
        self.start_button.place(x=50, y=30)
        self.finish_button = tk.Button(
            self, text="Finish", width=7, command=self.master.destroy, bg=background_color, fg=font_color)
        self.finish_button.place(x=115, y=30)
        self.console = tk.LabelFrame(
            self, text="Console", height=140, width=300, bg=background_color, fg=font_color)
        self.console.place(x=0, y=60)
        self.console_life_label = tk.Label(
            self.console, text="Life:", bg=background_color, fg=font_color)
        self.console_life_label.place(x=0, y=20)
        self.console_life = tk.Label(
            self.console, text="300", bg=background_color, fg=font_color)
        self.console_life.place(x=30, y=20)
        self.console_points_label = tk.Label(
            self.console, text="Point:", bg=background_color, fg=font_color)
        self.console_points_label.place(x=0, y=40)
        self.console_points = tk.Label(
            self.console, text="0", bg=background_color, fg=font_color)
        self.console_points.place(x=35, y=40)
        self.collide_label = tk.Label(
            self.console, text=" ", bg=background_color, fg=font_color)
        self.collide_label.place(x=0, y=60)
        self.name_label = tk.Label(
            self.console, text="Please, enter your name", bg=background_color, fg=font_color)
    ...
    def Jerry_Game(self):
        ...
        while self.game_over == False:
            ...
            self.enemy_Character_1.update(self.counter_mov_enemy, 1)
            # para entender esto mirar el metodo "update" de la clase "Enemy"
            self.enemy_Character_2.update(self.counter_mov_enemy, 2)
            # para entender esto mirar el metodo "update" de la clase "Enemy"
            self.enemy_Character_3.update(self.counter_mov_enemy, 3)
            ...
            if self.left_button.read() == True:
                # para entender esto mirar el metodo "update" de la clase "Jerry"
